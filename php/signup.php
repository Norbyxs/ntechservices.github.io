...
if(!empty($krstnémeno) && !empty($priezvisko) && !empty($email) && !empty($telc) && !empty($password) && !empty($cpassword)){
    //if email is valid
    if(filter_var($email,FILTER_VALIDATE_EMAIL)){
        ...
        if(mysqli_num_rows($sql) > 0){
            ...
            if(in_array($img_extension,$extensions) === true){
                ...
                if(move_uploaded_file($tmp_name,"../uložené-fotky/" . $newimagename)){
                    $random_id = rand(time(),10000000);
                    $otp =  mt_rand(1111,9999);
                    // let's start insert data into table

                    $sql2 = mysqli_query($conn,"INSERT INTO users (unique_id, krstnémeno, priezvisko, email, telc, password, image,otp,verification_status, Role)
                    VALUES ({$random_id},'{$krstnémeno}','{$priezvisko}','{$email}','{$telc}','{$password}','{$newimagename}','{$otp}','{$verification_status}','{$Role}')");
                    if($sql2){
                        ...
                        if(mysqli_num_rows($sql3)>0){
                            ...
                            if($otp){
                                $receiver = $email;
                                $subject = "From: $krstnémeno $priezvisko <$email>";
                                $body = "Name "." $krstnémeno $priezvisko  \n Email" . " $email \n Otp" . " $otp";
                                $sender = "From: NTech.services99@gmail.com" ; 
